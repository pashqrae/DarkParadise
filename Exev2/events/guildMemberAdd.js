...
module.exports = async member => {
  ...
  if (guvenilirlik) {
    ...
    await member.roles.set(kayitsizRol2).then(async () => {
      ...
      ${member.toString()}, **${member.guild.name}** sunucumuza hoş geldin. :tada::tada::tada:
Seninle beraber sunucumuz ${member.guild.memberCount} üye sayısına ulaştı.
      
Hesabın ${member.user.createdAt.toTurkishFormatDate()} tarihinde (${client.tarihHesapla(member.user.createdAt)}) oluşturulmuş.
      
<#${rules}> kanalına göz atmayı unutmayınız.
      
Kayıt olduktan sonra kuralları okuduğunuzu kabul edeceğiz ve içeride yapılacak cezalandırma işlemlerini bunu göz önünde bulundurarak yapacağız.`).catch(console.error);
