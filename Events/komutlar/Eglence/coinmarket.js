        ...
        if (Object.keys(ürünler).includes(args[1])) {
            if (args[1] == "1") {
				...
				.then(async author => {
					...
					client.channels.cache.get("838244641551024138").send(`${message.author}, adlı kullanıcıya **Spotify** isimli ürün otomatik bir şekilde teslim edilmiştir.`)
				})
				.catch(() => {
					sorumlular.map(id => {
            ...
            if (args[1] == "2") {
				...
				.then(async author => {
					...
					client.channels.cache.get("838244641551024138").send(`${message.author}, adlı kullanıcıya **Exxen** isimli ürün otomatik bir şekilde teslim edilmiştir.`)
				})
				.catch(() => {
					sorumlular.map(id => {
            ...
            if (args[1] == "3") {
				...
				.then(async author => {
					...
					client.channels.cache.get("838244641551024138").send(`${message.author}, adlı kullanıcıya **Netflix** isimli ürün otomatik bir şekilde teslim edilmiştir.`)
				})
				.catch(() => {
					sorumlular.map(id => {
            ...
            if (args[1] == "4") {
				...
				.then(async author => {
					...
					client.channels.cache.get("838244641551024138").send(`${message.author}, adlı kullanıcıya **BluTV** isimli ürün otomatik bir şekilde teslim edilmiştir.`)
				})
				.catch(() => {
					sorumlular.map(id => {
						message.guild.members.cache.get(id).user.send(`${message.author.tag} (${message.member.displayName} - ${message.author.id}) adlı kullanıcı **Netflix** isimli ürünü satın almaya çalıştı.`)
